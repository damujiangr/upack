  ...
  return rootComponent.getLocalDependenciesTree()
    .then(function (filledTree) {
      ...
      if (!options.force) {
        ...
        parsedList = parsedList.filter(function (parsed) {
          ...
          var semantic = upack.stringify(_(parsed).omit('version').value());

          if (!matched) {
            upack.logger.info(upack.i18n('COMPONENT_NOT_INSTALLED', semantic));
          }
        ...
        var promise = children.length ? specifiedComponent.getLocalDependenciesTree() : upack.async(specifiedComponent.tree);
          ...
          .then(function (filledUninstallTree) {
            ...
            if (parsedList.length) {
              ...
              parsedList.forEach(function (parsed) {
                var semantic = upack.stringify(_(parsed).omit(['version']).value());
                upack.logger.info(upack.i18n('OTHER_COMPONENT_DEPEND', semantic));
              });
              throw new Error(upack.i18n('UNABLE_TO_UNINSTALL'));
            }
            ...
            var restList = toUninstallAll.filter(function (parsed) {
              ...
              if (matched) {
                upack.logger.info(upack.i18n('OTHER_COMPONENT_DEPEND', parsed.owner + '/' + parsed.name));
              }
    ...
    .then(function (uninstalledList) {
      ...
      if (!uninstalledList.length) {
        upack.logger.info(upack.i18n('NO_COMPONENT_TO_UNINSTALL'));
        return uninstalledList;
      }
      ...
      upack.logger.info(upack.i18n('UNINSTALLED_COMPONENTS', str + tree));
