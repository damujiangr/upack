...
function buildConflictList(treeModel, installed) {
  ...
  if (conflictList.length > 0) {
    var err = new Error(upack.i18n('COMPONENT_OWNERS_CONFLICT'));
...
function resolveConflictByAsk(treeModel, installed) {
  ...
  _.forIn(conflictList, function (components, semantic) {
    ...
    choices = components.map(function (component) {
      return {
        name: component.version + ' ($0)'.replace('$0', upack.i18n((component.type === 'local' ? 'INSTALLED' : 'TO_INSTALL'))),
  ...
  if (choices.length) {
    ...
    upack.logger.info(upack.i18n('COMPONENT_VERSION_CONFLICT', conflicted));
...
function resolveConflictByLatest(treeModel, installed) {
  ...
  if (resolved) {
    upack.logger.info(upack.i18n('MERGE_CONFLICT_VERSIONS', conflicted.yellow, conflictVersions.red, resolved.version.yellow));
    return upack.async(resolved)
