...
var Component = function (parsed, type, options) {
  ...
  if ('root' === type) {
    parsed = {
      name: upack.config.name,
      owner: upack.profile.username,
      version: upack.config.version
    };
  }
  ...
  this.tree = null;
  this.exclude = [];
  this.fullPath = path.join(upack.componentRoot, parsed.name);
...
Component.prototype = {
  ...
  getLocalDependenciesTree: function (callback) {
    ...
    function getDependencies(parsed, isRoot) {
      var componentDir = isRoot ? path.join(upack.root) : path.join(upack.componentRoot, parsed.name);
      ...
      dependenciesMap = dependenciesMap.filter(function (semantic) {
        var subParsed = upack.parse(semantic);
        return !(subParsed.hasOwnProperty('resolved') && subParsed.resolved);
      });

      tree.subNodes = dependenciesMap.map(function (semantic) {
        var subParsed = upack.parse(semantic);
        var subComponentJson = path.join(upack.componentRoot, subParsed.name, 'component.json');
        ...
        if (!fs.existsSync(subComponentJson)) {
          ...
          throw new Error(upack.i18n('COMPONENT_NOT_INSTALLED', subParsed.owner + '/' + subParsed.name));
        }
    ...
    var getChildren = function (tree) {
      ...
      var isRoot = (parsed.name === upack.config.name && parsed.owner === upack.profile.username);
  ...
  getRemoteDependenciesTree: function (callback) {
    ...
    var getChildren = function (tree) {
      ...
      return gitlabRepo.getDependencies(parsed.owner, parsed.name, parsed.version)
        .then(function (map) {
          tree.props = _.assign({}, tree.props, upack.parse(map.name));
          tree.subNodes = map.dependencies.map(function (semantic) {
            return {
              props: upack.parse(semantic)
            };
          });
  ...
  checkUpdate: function () {
    ...
    return gitlabRepo.getLatestTag(self.owner, self.name)
      .then(function (tag) {
        ...
        self.status = self.hasUpdate ? self.latest.yellow : upack.i18n('NO_UPDATE_AVAILABLE');
        return self;
      });
  },
  validate: function () {
  ...
  install: function () {
    ...
    archivePath = path.join(upack.tmpDir, prefix + '-archive.zip');

    var promise = fs.existsSync(archivePath) ?
        upack.async(archivePath) :
        gitlabRepo.getArchive(self.owner, self.name, self.version);

    return promise
      .then(function (filePath) {
        var zip = new AdmZip(filePath);
        var tmpDir = path.join(upack.tmpDir, '.tmp' + Date.now());
        var componentDir = path.join(upack.componentRoot, self.name);
        ...
        if (!zipEntries.some(function (zipEntry) {
          ...
          throw new Error(upack.i18n('READING_EXTRACTED_FILE_ERROR'));
        }
  ...
  uninstall: function () {
    ...
    if (!self.installed) {
      return upack.async(self);
    }

    if (!fs.existsSync(dir)) {
      throw new Error(upack.i18n('COMPONENT_DIR_NOT_EXISTS', self.name));
    }
