...
var interceptors = function (strategy, installed, toInstall) {
  var strategies = {
    'latest': function (tree) {
      ...
      if (toMatch && !(node.hasOwnProperty('specified') && node.specified)) {
        node.merged = true;
        node.status = upack.i18n('MERGED_STATUS', toMatch.version);
      ...
      if (node.specified) {
        ...
        if (matched.version === node.version) {
          node.installed = true;
          node.status = upack.i18n('INSTALLED_STATUS');
          tree[walkable.childrenName] = [];
          return false;
        }
...
module.exports = function (toInstall, installed, strategy) {
  // upack.logger.info(upack.i18n('BEGIN_TO_ANALYZE_DEPENDENCIES'));
  var interceptor;
  ...
  var spinner = new Spinner(upack.i18n('ANALYZING_DEPENDENCIES'));
  ...
  return rootComponent.getRemoteDependenciesTree(interceptor)
    .then(function (filledTree) {
      ...
      nodes.forEach(function (parsed) {
        if (parsed.installed) {
          upack.logger.info(upack.i18n('COMPONENT_ALREADY_INSTALLED', upack.stringify(_.omit(parsed, ['status']))));
        } else if (parsed.conflict) {
          upack.logger.info(upack.i18n('COMPONENT_VERSION_CONFLICT', upack.stringify(_.omit(parsed, ['status']))));
        }
      });
