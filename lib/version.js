...
module.exports = function (releaseType, options) {
  var rootDir = upack.root; // Root directory
  var componentJson;
  var json;
  var repo;
  var version = upack.config.version || '0.0.0';
  var nextVersion;
  ...
  return Git.Repository.openExt(path.resolve(upack.root), 0, '')
    ...
    .then(function (tags) {
      ...
      if (!tags.length) { // If not published before
        upack.logger.info('No version published before!');
        ...
        upack.logger.info(upack.i18n('CURRENT_VERSION', version));
      }
    ...
    .then(function (answers) {
      ...
      return repo.createCommitOnHead(['component.json'], author, committer, 'v' + nextVersion)
        .then(function (oid) {
          upack.logger.info("New Commit: " + oid.toString());
          return repo.createTag(oid, nextVersion, answers.comment);
        })
        .then(function (tag) {
          upack.logger.info("New Version: " + nextVersion);
          upack.logger.info("Release message: " + answers.comment);
        });
